...
class NDT2Tokenizer:
    ...
    def bin_spikes(self, t, num_bins=None, bin_time=None):
        if len(t) == 0:
            return np.zeros(self.num_bins, dtype=np.int32)

        num_bins = num_bins or self.num_bins
        bin_time = bin_time or self.bin_time

        ans = np.bincount(
            np.floor(t / self.bin_time).astype(np.int32), minlength=self.num_bins
        )
        if len(ans) == self.num_bins + 1:
            # sometimes spikes are at the end of the context time due to floating point errors
            ans, extra = ans[:-1], ans[-1]
            ans[-1] += extra

        assert (
            len(ans) == self.num_bins
        ), f"{len(ans)} != {self.num_bins}\n{t}, {self.bin_time}, {self.num_bins}"
        return ans

    def __call__(self, data):
        # -- Spikes
        # bin and pad space-dimension if necessary
