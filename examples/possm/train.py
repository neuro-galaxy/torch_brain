...
from torch_brain.models import POSSM
...
class TrainWrapper(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_size = batch["model_inputs"]["spike_unit_index"].shape[0]
...
class DataModule(L.LightningDataModule):
    ...
    def setup_dataset_and_link_model(self, model: POSSM):
    ...
    def _init_model_vocab(self, model: POSSM):
        if model.unit_emb.is_lazy():
            model.unit_emb.initialize_vocab(self.get_unit_ids())
        if model.session_emb.is_lazy():
    ...
    def val_dataloader(self):
        ...
        val_trial_sampler = TrialSampler(
            sampling_intervals=self.val_dataset.get_sampling_intervals(),
            shuffle=False,
        )
        val_sampler = DistributedEvaluationSamplerWrapper(val_trial_sampler)
        val_sampler.set_params(
            num_replicas=self.trainer.world_size,
            rank=self.trainer.global_rank,
        )
        self.val_sequence_index = torch.arange(len(val_sampler))
    ...
    def test_dataloader(self):
        ...
        test_trial_sampler = TrialSampler(
            sampling_intervals=self.test_dataset.get_sampling_intervals(),
            shuffle=False,
        )
        test_sampler = DistributedEvaluationSamplerWrapper(test_trial_sampler)
        test_sampler.set_params(
            num_replicas=self.trainer.world_size,
            rank=self.trainer.global_rank,
        )
        self.test_sequence_index = torch.arange(len(test_sampler))
