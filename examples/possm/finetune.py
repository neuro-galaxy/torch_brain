...
from torch_brain.models.possm import POSSM
...
class GradualUnfreezing(L.Callback):
    ...
    The only parts of the model that will be left unfrozen are the unit and session
    embeddings. Once we reach the specified epoch (`unfreeze_at_epoch`), the entire
    model will be unfrozen.
    """
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        if self.enabled and (trainer.current_epoch == self.unfreeze_at_epoch):
            ...
            self.cli_log.info(f"POSSM unfrozen at epoch {trainer.current_epoch}")
