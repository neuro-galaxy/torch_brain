...
class CustomIBLSampler(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        *,
        sampling_intervals: Dict[str, Interval],
        extra_sampling_intervals: Dict[str, Interval],
        window_length: float,
        extra_sampling_factor: float = 1.0,
        generator: Optional[torch.Generator] = None,
        drop_short: bool = True,
    ):
        self.sampling_intervals = sampling_intervals
        self.extra_sampling_intervals = extra_sampling_intervals
        self.extra_sampling_factor = extra_sampling_factor
        self.window_length = window_length
        self.generator = generator
        self.drop_short = drop_short
    ...
    def _estimated_len(self):
        ...
        if self.drop_short and total_short_dropped > 0:
            ...
            if num_samples == 0:
                raise ValueError("All intervals are too short to sample from.")
        return num_samples + int(num_samples * self.extra_sampling_factor)
    ...
    def __iter__(self):
        ...
        indices_extra = []
        for session_name, sampling_intervals in self.extra_sampling_intervals.items():
            for start, end in zip(sampling_intervals.start, sampling_intervals.end):
                interval_length = end - start
                if interval_length < self.window_length:
                    if self.drop_short:
                ...
                if len(indices_) > 0:
                    indices_extra.extend(indices_)
                    right_offset = end - indices_extra[-1].end
                else:
                    right_offset = end - start - left_offset
        ...
        # sample extra indices
        num_extra_samples = int(len(indices) * self.extra_sampling_factor)
        for idx in torch.randperm(len(indices_extra), generator=self.generator):
            if num_extra_samples > 0:
                indices.append(indices_extra[idx])
                num_extra_samples -= 1
            else:
                break
