...
class POCO(nn.Module):
    ...
    def __init__(self, config: NeuralPredictionConfig, input_size):
        ...
        self.readout_spec = MODALITY_REGISTRY[modality_name]
        assert config.decoder_type == "POYO"
        self.decoder = POCOPOYO(
            sequence_length=1.0,  # maybe the self.Tin not sure
            latent_step=1.0 / 8,
            readout_spec=self.readout_spec,
    ...
    def tokenize(self, data):
        print(data)
        out = data.reshape(data.shape[0], self.Tin // self.T_step, self.T_step)
...
if __name__ == "__main__":
    ...
    x_list = [
        torch.randn(48, 2, 5, dtype=torch.bfloat16, device=device),
        torch.randn(48, 2, 10, dtype=torch.bfloat16, device=device),
    ]
