...
class POSSM(nn.Module):
    ...
    Args:
        ...
        t_min: Minimum time period for encoder rotary embeddings (spike timing).
        t_max: Maximum time period for encoder rotary embeddings.
        dec_t_min: Minimum time period for decoder rotary embeddings (bin timing).
            Defaults to 0.01 (10 ms), sufficient for 50 ms bin spacing while
            maintaining good float32 precision at large absolute timestamps.
        dec_t_max: Maximum time period for decoder rotary embeddings.
    """
    ...
    def forward(
        ...
        *,
        ...
        # Decoder cross-attention with per-query gathered context and relative
        # timestamps (matches the OnlinePOSSM convention exactly).
        N_out = output_queries.shape[1]
        ctx_lim = self.output_ca_ctx_lim
        rnn_dim = outputs.shape[-1]
        device = outputs.device

        # Padded outputs (bin_index=0) clamped to 1; ignored downstream
        # since no readout spec has id=0.
        bin_idx = output_bin_index.clamp(min=1)  # [B, N_out], 1-based

        # Context positions (0-indexed) for each query: [B, N_out, ctx_lim]
        offsets = torch.arange(-ctx_lim, 0, device=device)
        ctx_pos = bin_idx.unsqueeze(-1) + offsets
        valid = ctx_pos >= 0
        ctx_pos = ctx_pos.clamp(min=0)

        # Gather per-query context from GRU outputs: [B, N_out, ctx_lim, D]
        ctx = outputs.gather(
            1, ctx_pos.reshape(B, -1, 1).expand(-1, -1, rnn_dim)
        ).reshape(B, N_out, ctx_lim, rnn_dim)

        # Relative timestamps — same fixed values as OnlinePOSSM:
        #   context: [-(ctx_lim-1)*bw, ..., 0]
        #   query:   offset from its bin end (typically 0)
        ctx_rel_ts = (
            torch.arange(-(ctx_lim - 1), 1, dtype=torch.float32, device=device)
            * self.bin_width
        )
        query_rel_ts = output_timestamps - bin_idx.float() * self.bin_width

        # Fold each query into its own batch element: [B*N_out, ...]
        BN = B * N_out
        q_emb = self.dec_rotary_emb(query_rel_ts.reshape(BN, 1))
        c_emb = self.dec_rotary_emb(ctx_rel_ts.unsqueeze(0).expand(BN, -1))

        l = output_queries.reshape(BN, 1, -1) + self.dec_atn(
        ...
        output_latents = (l + self.dec_ffn(l)).reshape(B, N_out, -1)
    ...
    def tokenize(self, data: Data) -> Dict:
        ...
        start = 0
            ...
            data.domain.end[0] - data.domain.start[0]
        ...
        data_dict = {
            "model_inputs": {
                ...
                "input_mask": pad2d(input_mask_2d),
