...
class CrossAttention(nn.Module):
    """Multi-head cross-attention layer, including the query/key/value weights.

    This layer performs cross-attention between a query sequence and a context sequence.
    It first normalizes the inputs, projects them to query/key/value space, applies attention,
    ...
    - forward_varlen(): Uses sequence lengths instead of masks for sequences that are chained
      together in a single batch dimension. This can be more memory efficient and quicker since
      it avoids padding and masking, but requires the sequences to be concatenated rather
      than stacked.
...
def attn_pytorch_func(
    *,
    ...
    r"""Wraps the default attention implementation.
...
def attn_xformers_func(
    *,
    ...
    r"""Wraps the memory efficient attention implementation
...
def attn_xformers_varlen_func(
    *,
    ...
    r"""Wraps the memory efficient attention implementation for sequences
    with variable sequence length
