...
class SelectByRegion:
    ...
    Args:
        ...
        reset_index (bool, optional): Whether to reset the index of the units. Set this to False
            if you want to set the activity of the dropped units to 0, and to True if you
            want to reindex the units to create a new sub-population. Defaults to True.
    """
    ...
    def __call__(self, data: Data):
        ...
        if not isinstance(target_obj, IrregularTimeSeries):
            raise ValueError(
                f"Unsupported type for {self.field}: {type(target_obj)}. Only IrregularTimeSeries is supported."
            )
