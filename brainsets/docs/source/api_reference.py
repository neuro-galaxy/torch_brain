...
e.g., `brainsets.pipeline`, the hook would be `pipeline_ref`; for a section, e.g.,
"Functions" under `brainsets.core`, the hook would be `core_ref-functions`. However,
note that a better way is to refer using the :mod: directive, e.g.,
:mod:`brainsets.pipeline` for the module. Only in case that a section is not a
