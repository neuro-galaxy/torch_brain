...
def test_multiple_regular_time_series():
    ...
    np.testing.assert_array_equal(
        patched_data.lfp1.timestamps, patched_data.lfp2.timestamps
    )
...
def test_mixed_data_types(irregular_and_regular_data):
    ...
    np.testing.assert_array_equal(
        patched_data.spikes.timestamps, irregular_and_regular_data.spikes.timestamps
    )
    np.testing.assert_array_equal(
        patched_data.spikes.unit_index, irregular_and_regular_data.spikes.unit_index
    )

    # ArrayDict should be preserved
    assert hasattr(patched_data, "units")
    np.testing.assert_array_equal(
        patched_data.units.id, irregular_and_regular_data.units.id
    )
...
def test_irregular_time_series_irregularly_spaced():
    ...
    np.testing.assert_array_equal(patched_data.irregular_ts.timestamps, timestamps)
