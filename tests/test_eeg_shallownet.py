...
from torch_brain.models.eeg_shallownet import ShallowNet
...
def make_temporaldata_eeg_sample(
    ...
    # Trials: we only need a 'label' field that ShallowNet.tokenize can read
    # plus timekeys for start/end so slicing remains sane.
    trials = Interval(
        start=np.array([0.0]),
